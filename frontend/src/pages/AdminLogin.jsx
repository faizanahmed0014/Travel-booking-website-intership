...
const AdminLogin = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setToast({ message: error.response?.data?.message || 'Admin login failed', type: 'error' });
    }
  };
  ...
  return (
    <main className="mx-auto max-w-md px-4 py-14 sm:px-6">
      ...
      <div className="rounded-4xl border border-slate-200 bg-white p-8 shadow-soft">
        ...
        <p className="mt-2 text-slate-600">Use your admin credentials to post and manage hotel and flight articles.</p>
        <form onSubmit={handleSubmit} className="mt-8 space-y-5">
          ...
          <button type="submit" className="w-full rounded-full bg-slate-900 px-5 py-3 text-sm font-semibold text-white transition hover:bg-slate-700">Login as admin</button>
        </form>
        <p className="mt-5 text-center text-sm text-slate-600">Back to <Link to="/login" className="font-semibold text-slate-900">User login</Link></p>
      </div>
    </main>
  );
};
